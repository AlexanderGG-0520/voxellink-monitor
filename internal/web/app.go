...
const pageStyles = `
*{box-sizing:border-box}body{margin:0;background:#0b1020;color:#e8edf7;font:16px/1.55 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}main{width:min(100% - 32px,960px);margin:0 auto;padding:48px 0 64px}h1,h2,h3,p{margin-top:0}h1{margin-bottom:6px;font-size:clamp(1.8rem,5vw,2.5rem);letter-spacing:-.04em}h2{margin:0;font-size:1.18rem;letter-spacing:-.015em}h3{font-size:.82rem;letter-spacing:.08em;text-transform:uppercase;color:#aab6ca}.eyebrow{margin:0 0 20px;color:#9cabca;font-size:.9rem}.card{margin:14px 0;padding:22px;border:1px solid #26324a;border-radius:16px;background:linear-gradient(145deg,#182238,#121a2a);box-shadow:0 10px 28px rgb(0 0 0 / .15)}.card-head{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:14px}.status{display:inline-flex;align-items:center;gap:8px;margin:0;padding:5px 10px;border:1px solid #345078;border-radius:999px;background:#14243a;color:#b9d9ff;font-size:.78rem;font-weight:750;letter-spacing:.045em}.status::before{content:"";width:8px;height:8px;border-radius:50%;background:#8ba1bd;box-shadow:0 0 0 4px rgb(139 161 189 / .13)}[data-status="OPERATIONAL"] .status{border-color:#227150;background:#102d27;color:#a7f2cb}[data-status="OPERATIONAL"] .status::before{background:#43dc91;box-shadow:0 0 0 4px rgb(67 220 145 / .14)}[data-status="DEGRADED"] .status{border-color:#8a651c;background:#322712;color:#ffe29a}[data-status="DEGRADED"] .status::before{background:#ffbd3f}[data-status="OUTAGE"] .status{border-color:#943d4e;background:#381923;color:#ffb8c2}[data-status="OUTAGE"] .status::before{background:#ff667a}[data-status="MAINTENANCE"] .status{border-color:#5b57a2;background:#201e42;color:#d0ceff}[data-status="MAINTENANCE"] .status::before{background:#aaa5ff}[data-status="UNKNOWN"] .status{border-color:#59677e;background:#1d2532;color:#d2d9e6}.meta{margin:0;color:#aeb9cc;font-size:.91rem}.alert{margin:16px 0;padding:12px 14px;border-left:3px solid #ffbd3f;border-radius:8px;background:#322712;color:#ffe7ad}.report-form,.settings-form{display:flex;align-items:end;gap:10px;flex-wrap:wrap;margin-top:20px;padding-top:18px;border-top:1px solid #29364d}label{display:grid;gap:5px;font-size:.86rem;color:#bdc8da}input,select,button{min-height:42px;border-radius:9px;font:inherit}input,select{min-width:170px;padding:8px 10px;border:1px solid #3a4963;background:#0e1626;color:#f1f5ff}input:focus,select:focus{outline:2px solid #78b5ff;outline-offset:2px}button{padding:8px 14px;border:1px solid #528dcc;background:#1672bd;color:white;font-weight:700;cursor:pointer}button:hover{background:#2285d6}small{display:block;margin-top:11px;color:#9daac0;font-size:.78rem}.metrics{display:flex;gap:10px;flex-wrap:wrap;margin:16px 0}.metric{min-width:150px;padding:10px 12px;border-radius:10px;background:#0e1727;color:#aebbd0;font-size:.82rem}.metric strong{display:block;color:#f0f5ff;font-size:1rem}.incidents{margin:0;padding-left:18px;color:#bfc9da}.incidents li+li{margin-top:7px}.subtle{color:#aeb9cc}@media(max-width:600px){main{width:min(100% - 24px,960px);padding-top:28px}.card{padding:17px;border-radius:13px}.card-head{align-items:flex-start;flex-direction:column;gap:10px}.report-form,.settings-form{align-items:stretch;gap:9px}.report-form>* ,.settings-form>*{width:100%}input,select,button{width:100%}}
`

const homeTemplate = `<!doctype html><html lang="ja"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>VoxelLink Monitor</title><style>` + pageStyles + `</style><main><h1>サーバーステータス</h1><p class="eyebrow">VoxelLink掲載Minecraft Javaサーバーの稼働状況</p>{{if .Servers}}{{range .Servers}}<section class="card" data-status="{{.Status}}"><div class="card-head"><h2>{{.Name}}</h2><p class="status">{{.Status}}</p></div><p class="meta">{{if .LastOutcome}}最終確認 {{.LastCheckedAt}}{{else}}まだ確認結果がありません{{end}}</p>{{if .Crowd.Anomalous}}<p class="alert">接続トラブルの報告が通常より増えています（直近15分: {{.Crowd.Reports}}件）。</p>{{end}}<form class="report-form" method="post" action="/servers/{{.ID}}/reports"><label>接続トラブルを報告<select name="report_type"><option value="CONNECTION">接続できない</option><option value="LOGIN">ログインできない</option><option value="TIMEOUT">タイムアウト</option><option value="LAG">ラグ・遅延</option><option value="OTHER">その他</option></select></label><label>補足（任意）<input name="detail" maxlength="280" placeholder="280文字まで"></label><button>報告を送信</button></form><small>同じサーバーへの報告は1日1件まで集計されます。IPアドレス等は保存しません。</small></section>{{end}}{{else}}<section class="card"><p>現在、公開中の監視対象はありません。</p></section>{{end}}<p class="subtle"><a href="/login">Discordで管理画面へログイン</a></p></main></html>`

const consoleTemplate = `<!doctype html><html lang="ja"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>VoxelLink Monitor Console</title><style>` + pageStyles + `</style><main><h1>あなたの監視サーバー</h1><p class="eyebrow">通知とメンテナンス予定を管理できます</p>{{if .Servers}}{{range .Servers}}<section class="card" data-status="{{.Snapshot.Status}}"><div class="card-head"><h2>{{.Snapshot.Name}}</h2><p class="status">{{.Snapshot.Status}}</p></div><p class="meta">{{.Snapshot.Hostname}}:{{.Snapshot.Port}} · {{.Snapshot.Transport}}</p><div class="metrics"><span class="metric">直近24時間<strong>{{printf "%.2f" .Uptime24h}}%</strong></span>{{if .Snapshot.LastOutcome}}<span class="metric">最終確認<strong>{{.Snapshot.LastCheckedAt}}{{if .Snapshot.Latency}} · {{.Snapshot.Latency.Milliseconds}}ms{{end}}</strong></span>{{end}}</div><h3>直近の障害</h3>{{if .Incidents}}<ul class="incidents">{{range .Incidents}}<li>{{.StartedAt}} — {{.State}}（{{.Reason}}）</li>{{end}}</ul>{{else}}<p class="subtle">直近の障害はありません。</p>{{end}}<form class="settings-form" method="post" action="/console/servers/{{.Snapshot.ID}}/enabled"><input type="hidden" name="enabled" value="{{if .Snapshot.Enabled}}false{{else}}true{{end}}"><button>{{if .Snapshot.Enabled}}監視を無効化{{else}}監視を有効化{{end}}</button></form><form class="settings-form" method="post" action="/console/servers/{{.Snapshot.ID}}/channel"><label>Discord ステータスチャンネルID<input name="channel_id" required></label><button>通知先を保存</button></form><form class="settings-form" method="post" action="/console/servers/{{.Snapshot.ID}}/maintenance"><label>メンテ開始（JST）<input type="datetime-local" name="starts_at" required></label><label>終了（JST）<input type="datetime-local" name="ends_at" required></label><button>メンテナンスを予定</button></form></section>{{end}}{{else}}<section class="card"><p>管理できるVoxelLink掲載サーバーはまだありません。</p></section>{{end}}</main></html>`
