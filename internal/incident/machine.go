...
func (m *Machine) Apply(r domain.CheckResult) (domain.PublicStatus, bool) {
	if r.Outcome == domain.ProbeError {
		changed := m.state != domain.Unknown
		m.state = domain.Unknown
		m.failures = 0
		m.successes = 0
	...
	if r.Outcome == domain.Success {
		m.failures = 0
		if m.state == domain.Outage {
			m.successes++
			if m.successes < 2 {
				return m.state, false
			}
			m.state = domain.Operational
			m.successes = 0
			return m.state, true
		}
		m.successes = 0
		if m.state != domain.Operational {
			m.state = domain.Operational
			return m.state, true
		}
		return m.state, false
	...
	m.successes = 0
	m.failures++
	if m.failures >= 3 && m.state != domain.Outage {
		m.state = domain.Outage
		return m.state, true
	}
	return m.state, false
}
